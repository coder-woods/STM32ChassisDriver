...
/****************************************************************************
* 名    称: u8 usmart_strcmp(u8 *str1,u8 *str2)
* 功    能：对比字符串str1和str2
* 入口参数：*str1:字符串1指针 ; *str2:字符串2指针
* 返回参数：返回值:0，相等;1，不相等;
* 说    明：
* 作    者：嵌入式小白Woods
****************************************************************************/
...
/****************************************************************************
* 名    称: void usmart_strcopy(u8*str1,u8 *str2)
* 功    能：把str1的内容copy到str2
* 入口参数：*str1:字符串1指针 ; *str2:字符串2指针
* 返回参数：返回值:0，相等;1，不相等;
* 说    明：
* 作    者：嵌入式小白Woods
...
* 名    称: u8 usmart_strlen(u8*str)
* 功    能：把字符串转为数字
* 入口参数：*str:数字字符串指针；*res:转换完的结果存放地址
* 返回参数：返回值:0，成功转换完成.其他,错误代码.
            1,数据格式错误.2,16进制位数为0.3,起始格式错误.4,十进制位数为0.
* 说    明：支持16进制转换,但是16进制字母必须是大写的,且格式为以0X开头的.
            不支持负数
* 作    者：嵌入式小白Woods
****************************************************************************/	
...
/****************************************************************************
* 名    称: u8 usmart_get_cmdname(u8*str,u8*cmdname,u8 *nlen,u8 maxlen)
* 功    能：得到指令名
* 入口参数：*str:源字符串 *cmdname:指令名 *nlen:指令名长度	maxlen:最大长度(做限制,指令不可能太长的)
* 返回参数：0,成功;其他,失败.	
* 说    明：
* 作    者：嵌入式小白Woods
****************************************************************************/	 
...
/****************************************************************************
* 名    称: u8 usmart_search_nextc(u8* str)
* 功    能：//获取下一个字符（当中间有很多空格的时候，此函数直接忽略空格，找到空格之后的第一个字符）
* 入口参数：str:字符串指针
* 返回参数：下一个字符	
* 说    明：
* 作    者：嵌入式小白Woods
...
/****************************************************************************
* 名    称: u8 usmart_get_fname(u8*str,u8*fname,u8 *pnum,u8 *rval)
* 功    能：从str中得到函数名
* 入口参数：*str:源字符串指针 *fname:获取到的函数名字指针 *pnum:函数的参数个数 *rval:是否需要显示返回值(0,不需要;1,需要)
...
/****************************************************************************
* 名    称: u8 usmart_get_aparm(u8 *str,u8 *fparm,u8 *ptype)
* 功    能：从str中得到一个函数的参数
* 入口参数：*str:源字符串指针 *fparm:参数字符串指针 *ptype:参数类型 0，数字;1，字符串;0XFF，参数错误
* 返回参数：0,已经无参数了;其他,下一个参数的偏移量.
* 说    明：
* 作    者：嵌入式小白Woods
****************************************************************************/	
...
/****************************************************************************
* 名    称: u8 usmart_get_parmpos(u8 num)
* 功    能：得到指定参数的起始地址
* 入口参数：num:第num个参数,范围0~9.
* 返回参数：该参数的起始地址
* 说    明：
* 作    者：嵌入式小白Woods
****************************************************************************/	
...
/****************************************************************************
* 名    称: u8 usmart_get_fparam(u8*str,u8 *parn)
* 功    能：从str中得到函数参数
* 入口参数：str:源字符串; parn:参数的多少.0表示无参数 void类型
* 返回参数：0,成功;其他,错误代码
* 说    明：
* 作    者：嵌入式小白Woods
****************************************************************************/	
